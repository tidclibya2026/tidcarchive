...
import { Archive, ArrowLeft, CheckCircle2, Clock3, FileInput, FilePlus2, Gauge, Hourglass, Inbox, TimerReset } from "lucide-react";
...
export default function Home() {
  ...
  const { data, isLoading } = trpc.dashboard.overview.useQuery(undefined, { refetchInterval: 30_000 });
  const metrics = data?.metrics;
  const cards = [
    { label: "معاملات نشطة", value: metrics?.active || 0, icon: Inbox, tone: "bg-[#e8f2f4] text-[#216379]", detail: "تحتاج متابعة أو إجراء" },
    { label: "قيد المعالجة", value: metrics?.inProgress || 0, icon: Hourglass, tone: "bg-[#fff3d9] text-[#a36e15]", detail: "لدى الإدارات المختصة" },
    { label: "متأخرة", value: metrics?.overdue || 0, icon: TimerReset, tone: "bg-[#fff0ed] text-[#b64c36]", detail: "تجاوزت تاريخ الاستحقاق" },
    { label: "منجزة", value: metrics?.completed || 0, icon: CheckCircle2, tone: "bg-[#e9f5ed] text-[#2f7d4b]", detail: "خلال الفترة الحالية" },
    { label: "مؤرشفة", value: metrics?.archived || 0, icon: Archive, tone: "bg-slate-100 text-slate-700", detail: "ضمن الأرشيف النهائي" },
  ];

  return (
    <DashboardLayout>
      <section className="mx-auto max-w-[1500px] space-y-6">
        <div className="relative overflow-hidden rounded-[1.7rem] bg-[#103548] px-6 py-7 text-white shadow-[0_20px_42px_rgba(16,53,72,.18)] md:px-8 md:py-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button onClick={() => setLocation("/outgoing?create=1")} variant="outline" className="h-11 border-white/20 bg-white/10 px-5 text-white hover:bg-white/20 hover:text-white"><FileInput className="ml-2 h-4 w-4" />تسجيل صادر</Button>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          {cards.map(card => (
            <div key={card.label} className="institution-card p-5">
              ...
              <p className="mt-4 text-[11px] text-[#78909a]">{card.detail}</p>
            </div>
          ))}
        </div>
